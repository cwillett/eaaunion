<?php
	...
	$tf = wp_remote_get("http://marketplace.envato.com/api/v3/new-files-from-user:virusnac,themeforest.json");
			...
			foreach ($json as $key => $themes) {
				if(sizeof($themes)){
					foreach ($themes as $cosmo_theme) {
						...
						echo '<div  class="our_themes left"><a href="'.$cosmo_theme->url.'?ref=virusnac"><img src="'.$cosmo_theme->thumbnail.'" alt="'.$cosmo_theme->item.'" /></a></div>';
					}
				}
			}
		}
	...
	$portfolio_link = 'http://themeforest.net/user/virusnac/portfolio?ref=virusnac';
