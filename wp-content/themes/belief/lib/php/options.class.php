<?php
    class options{
        ...
        function cosmothemes__general(){
            get_template_part('lib/templates/welcome_ message');
        }

        function cosmothemes__templates(){
        ...
        static function CallMenu( $name ) {
            ...
            if( !isset( $keys[ 2 ] ) ){
                $keys[ 2 ] = array_shift( array_keys( self::$menu[ $menu ][ $submenu ][ 'contains' ] ) );
            }
            list( ,, $tab ) = $keys;
		...
		function get_header( $menu, $submenu, $current ){
			$result = '';
