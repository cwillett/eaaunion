...
class LBElement{
    ...
    function render_frontend_delimiter(){ 
        ...
        }elseif ($this -> delimiter_type == 'doubleline-bold') {
            $rgb_color = $this->delimiter_text_color; 
            $delimiter_border = "border-top: 2px solid $rgb_color; border-bottom: 1px solid $rgb_color;";
        }elseif ($this -> delimiter_type == 'dashed') {
            $rgb_color = $this->delimiter_text_color; 
            $delimiter_border = "border-top: 1px dashed $rgb_color";
        }else{
            $delimiter_border = "";
        }
    ...
    function render_frontend_posts_grid_view_thumbnails_carousel( $posts ){
        ...
        foreach( $posts as $post ){
            ...
            post::grid_view_thumbnails($post, '', '', $filter_type = '', $taxonomy = 'category', $element_type = 'article', '', $this -> meta);
            echo '</div>';
    ...
    function render_frontend_posts_grid_view_carousel( $posts ){
        ...
        foreach( $posts as $post ){
            ...
            $hide_excerpt = ( $this -> enb_hide_excerpt == 'yes' );
            $show_meta = ( $this -> show_grid_view_meta == 'yes' );

            echo '<div class="'.$this -> columns_arabic_to_word( $this -> columns ).' columns">';
            
            call_user_func( array( 'post', $this -> view ), $post, '', '', $hide_excerpt, $element_type = 'article',$is_masonry = false, $is_carousel = true, $this -> meta);
            echo '</div>'; 
        }
    ...
    function render_frontend_posts_grid_view_thumbnails( $posts ){
        ...
        if(!($this ->  is_ajax)){
            ...
            echo '<div class=" row  '.$filter_class.' '.$masonry_class.' thumbs-list image-grid " id="ul-'.$ul_id.'" >';
        }   /*EOF if(!$this ->  is_ajax)*/
        ...
        foreach( $posts as $post ){
            ...
            call_user_func( array( 'post', $this -> view ), $post, $this -> columns_arabic_to_word( $this -> columns ) . ' columns', $additional_class, $filter_type = $filter_type_rand, $taxonomy, $element_type = 'article', $is_masonry = $is_masonry , $this -> meta);
            $counter++;
        }
    ...
    function render_frontend_posts_timeline_view( $posts, $timeline_elem_class = ' timeline-view-right-elem ' ){
        ...
        if(!($this ->  is_ajax)){
            echo '<div class=" row timeline-view list-view ">';
        ...
        if ($this -> enb_hide_excerpt == 'yes') {
            $hide_excerpt = true;
        }else { 
            $hide_excerpt = false; 
        }
        
        if ($this -> hide_timeline_img == 'yes') {
            $hide_img = true;
        }else { 
            $hide_img = false; 
        }
        ...
        foreach( $posts as $post ){
            ...
            post::timeline_view( $post, $hide_excerpt, $this -> meta, $hide_img);
        }
            echo '</div>';
        if(!($this ->  is_ajax)){
            echo '</div>';
    ...
    function render_frontend_posts_grid_view( $posts ){
        ...
        foreach( $posts as $post ){
            ...
            if( ($counter % $this -> columns) == 1 && $counter != 1 && $this -> enb_masonry != 'yes'){
                echo '<div class="clear"></div>';
            }            call_user_func( array( 'post', $this -> view ), $post, $this -> columns_arabic_to_word( $this -> columns ) . ' columns', $additional_class, $hide_excerpt, $element_type = 'article', $is_masonry = $is_masonry, $is_carousel = '', $this -> meta );
    ...
    function render_frontend_posts_list_view( $posts ){
        ...
        foreach( $posts as $post ){
            ...
            call_user_func( array( 'post', $this -> view ), $post, $this -> template, $additional_hidden_class_for_load_more, $this -> list_view_thumb_size, $show_full_content, $hide_excerpt, $is_timeline = '' , $this -> meta );
        }
    ...
    function render_frontend_teams(){
        ...
        foreach( $this -> teams as $term_id ){
            $term = get_term( $term_id, 'people-group' );
