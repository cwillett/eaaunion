<?php
    class menu extends Walker{
        ...
        function start_el(&$output, $item, $depth, $args) {
            ...
            if ($depth == 0){
                ...
                if($this -> limit == $this -> count){
                    ...
                    if($this -> menu_style == 'with_description'){ /*add description for more label if we have menu with description*/
                        $output .= '<span class="menu-description">'. __('Show more', 'cosmotheme' ) .'</span>';
                    }
                    $output .= '</a>';
            ...
            if( !empty( $item->attr_title ) && $this -> menu_style == 'with_description'){
                $item_output .= ' <span class="menu-description"> ';
            ...
            foreach( $terms as $key => $term ){
                ...
                if( $count < $this -> limit ){
                    if( $is_term && !$this -> issetcurrent ){
                            $menu .= '<li class="'.$item.'-item '.$this -> current.' no_description '.$this->nr_columns.'" id="'.$item.'-item-' . $id . '">';
                            $this -> issetcurrent = true;
                    }else{
                            $menu .= '<li class="'.$item.'-item '.$class.' no_description '.$this->nr_columns.'" id="'.$item.'-item-' . $id . '">';
                    }
                }else{
                    $menu .= '<li class="'.$item.'-item '.$class.' no_description '.$this->nr_columns.'" id="'.$item.'-item-' . $id . '">';
                }
