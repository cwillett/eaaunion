...
function paste1(contentID, element_id){
    ...
    jQuery( '.edit-header-background').click( function(){
        jQuery( '.header_slideshow_settings').addClass( 'hidden' );
        jQuery( '.header_bg_settings').removeClass( 'hidden' );
    });
    jQuery( '.edit-header-slideshow').click( function(){
        jQuery( '.header_bg_settings').addClass( 'hidden' );
        jQuery( '.header_slideshow_settings').removeClass( 'hidden' );
    });
    ...
    $( function(){
        ...
        $( '.admin-menu .add-new').click( function(){
            ...
            if (not_updated) {
                ...
                $( '.edit-header-background').click( function(){
                    $( '.header_slideshow_settings').addClass( 'hidden' );
                    $( '.header_bg_settings').removeClass( 'hidden' );
                });
                $( '.edit-header-slideshow').click( function(){
                    $( '.header_bg_settings').addClass( 'hidden' );
                    $( '.header_slideshow_settings').removeClass( 'hidden' );
                }); 
        ...
        $( '.layout-builder').on( 'show-hide', '.options-behavior, .options-orderby, .options-meta, .options-order', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
        ...
        $( '.layout-builder').on( 'show-hide', '.options-behavior', function( event ){
            ...
            if( 'featured' == $element_container.find( '.element-type input:checked').val() || 'latest' == $element_container.find( '.element-type input:checked').val() || 'latest_events' == $element_container.find( '.element-type input:checked').val() ||  'yes' == $element_container.find( '.masonry-options label input:checked').val() ){
...
/*            if($element_container.find( '.element-view-type input:checked').val() == 'news_view'){
        ...
        $( '.layout-builder').on( 'show-hide', '.options-list-view-excerpt', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val(); /*check the value for hide_excerpt option , we will use it to enable list view excerpt length only if the excerpt is enabled*/
            var hide_excerpt_setting = $element_container.find( '.hide_excerpt-options input:checked').val(); 
            $( event.currentTarget).toggle( hide_excerpt_setting == 'no' && ( element_type != 'empty' && element_type != 'delimiter' && element_type != 'textelement' && element_type != 'testimonials' && element_type != 'banners' && element_type != 'teams' && element_type != 'boxes' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'page' && element_type != 'event' && element_type != 'team' && ( $element_container.find( '.element-view-type input:checked' ).val() == 'list_view' ) ) );
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.hide_excerpt-options', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            var view_type = $element_container.find( '.element-view-type input:checked' ).val(); 
            $( event.currentTarget).toggle( view_type != 'grid_view_thumbnails' && !( element_type == 'teams' || element_type == 'event' || element_type == 'empty' || element_type == 'delimiter' || element_type == 'textelement' || element_type == 'boxes' || element_type == 'testimonials' || element_type == 'banners' || element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'boxes' || element_type == 'team' ) );
        });

        $( '.layout-builder').on( 'show-hide', '.hide_timeline_img-options', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            var view_types_accepted = ['category','tag','eventcategory','featured','latest','latest_events'];
            var view_type = $element_container.find( '.element-view-type input:checked' ).val(); 
            $( event.currentTarget).toggle( view_type == 'timeline_view' && jQuery.inArray( element_type, view_types_accepted) > -1 );
            console.log(view_type);
            console.log(jQuery.inArray( element_type, view_types_accepted));
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.option-numberposts', function( event ){
            ...
            var arr = ['widget_zone','post','page','event', 'banners','logo', 'searchbar','testimonials','copyright', 'breadcrumbs','textelement','socialicons', 'delimiter', 'empty'];
        ...
        $( '.layout-builder').on( 'show-hide', '.options-view-type', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( element_type != 'empty' && element_type != 'delimiter' && element_type != 'boxes' && element_type != 'textelement' && element_type != 'teams' && element_type != 'banners' && element_type != 'testimonials' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'event' && element_type != 'page' && element_type != 'team' );
        });

        $( '.layout-builder').on( 'show-hide', '.option-list-thumb-size, .option-list-title-position', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( !( element_type == 'empty' || element_type == 'delimiter' || element_type == 'textelement' || element_type == 'testimonials' || element_type == 'banners' || element_type == 'teams' || element_type == 'boxes' ||  element_type == 'widget_zone' || element_type == 'post' || element_type == 'event' || element_type == 'page' || element_type == 'team' /*|| ($element_container.find( '.element-view-type input:checked' ).val() != 'news_view')*/ || ($element_container.find( '.element-view-type input:checked' ).val() != 'list_view' || $element_container.find('.enb-list-thumbs-container input:checked').val() == 'yes' ) ) );
        });
