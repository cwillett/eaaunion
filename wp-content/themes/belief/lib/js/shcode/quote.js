...
jQuery(document).ready(function() {
	...
	jQuery('#quote_source').keyup(function() {
		
		if(jQuery.trim(jQuery(this).val()) != ''){
			jQuery('#quote_demo span.quote_source').html(jQuery(this).val());
		}
		else{
			jQuery('#quote_demo span.quote_source').html('John Doe');
		}
...
function insertQuote(){
	...
	if(jQuery('#quote_source').val() != ''){ 
		quote_source = jQuery('#quote_source').val();
	}
	...
	var quote_shcode = '[quote '+quote_style+' '+quote_float+' source="'+quote_source+'"]'+quote_content+'[/quote]';
