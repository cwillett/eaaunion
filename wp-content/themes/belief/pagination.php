<?php
	...
	if( is_search() ){
		...
		if( $wp_rewrite->using_permalinks() ){
			// $wtf = get_pagenum_link( 1 );
			// $wtf = remove_query_arg( 'fp_element', $wtf );
			// $wtf = remove_query_arg( 'fp_row', $wtf );
			// $wtf = remove_query_arg( 'fp_template', $wtf );
				//$pagination['base'] = user_trailingslashit( trailingslashit( remove_query_arg( array('fp_element', 'fp_type', 'fp_row' ) , remove_query_arg( 'type' , remove_query_arg( 's', get_pagenum_link( 1 ) ) ) ) ) . 'page/%#%/', 'paged' );
        }
